...
export const HospitalAnalytics: React.FC<HospitalAnalyticsProps> = ({ stats }) => {
  ...
  const demographics = useMemo(() => {
    return [
      { group: '0-18 yrs', percentage: 15, color: 'bg-medical-300' },
      { group: '19-35 yrs', percentage: 35, color: 'bg-medical-500' },
          ...
          <div className="flex items-center justify-between mb-8">
             <div>
               <h3 className="font-display text-xl font-black text-ink-800 flex items-center gap-2">
                 <TrendingUp className="text-medical-500" size={24} /> Weekly Trend
               </h3>
               <p className="text-ink-500 font-bold text-sm">Estimated traffic and revenue</p>
             </div>
          </div>
