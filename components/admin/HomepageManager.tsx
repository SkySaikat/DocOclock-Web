...
export const HomepageManager: React.FC<HomepageManagerProps> = ({ banners, onCreate, onUpdate, onDelete }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      {/* Header */}
      <div className="bg-white rounded-ds-lg p-6 shadow-ds-soft">
        <div className="flex items-center gap-4 mb-6">
          <div className="w-12 h-12 bg-medical-50 rounded-2xl flex items-center justify-center text-medical-600">
            <Image size={22} />
          </div>
          <div>
            <h2 className="text-xl font-display font-black text-ink-800">Homepage Slider</h2>
            <p className="text-sm font-bold text-ink-500">Manage hero banner slides shown to patients</p>
          </div>
        </div>

        {/* Add New Slide Form */}
        <div className="border border-ink-200 rounded-ds-md p-5 bg-ink-50/50 space-y-4">
          <h3 className="font-display font-black text-ink-700 flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Desktop Image */}
            <div>
              <label className="block text-xs font-black text-ink-600 uppercase tracking-widest mb-2 flex items-center gap-1.5">
              ...
              <button
                ...
                className="w-full flex flex-col items-center justify-center gap-2 border-2 border-dashed border-ink-300 hover:border-medical-400 rounded-xl p-4 transition-colors group"
              >
                {desktopPreview ? (
                  <img src={desktopPreview} alt="Desktop preview" className="h-24 w-full object-cover rounded-lg" />
                ) : (
                  <>
                    <Upload size={20} className="text-ink-400 group-hover:text-medical-500 transition-colors" />
                    <span className="text-xs font-bold text-ink-400 group-hover:text-medical-500 transition-colors">Click to upload</span>
                  </>
                )}
              </button>
            </div>
            ...
            <div>
              <label className="block text-xs font-black text-ink-600 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                <Smartphone size={12} /> Mobile Image <span className="text-ink-400 font-medium">(optional)</span>
              </label>
              ...
              >
                {mobilePreview ? (
                  ...
                  <>
                    <Upload size={20} className="text-ink-400 group-hover:text-medical-500 transition-colors" />
                    <span className="text-xs font-bold text-ink-400 group-hover:text-medical-500 transition-colors">Falls back to desktop</span>
                  </>
                )}
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-black text-ink-600 uppercase tracking-widest mb-1.5">Title (optional)</label>
              <input
                type="text"
                value={title}
                onChange={e => setTitle(e.target.value)}
                placeholder="e.g. Find Your Doctor"
                className="w-full px-4 py-2.5 rounded-ds-sm border border-ink-200 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-medical-500/20 focus:border-medical-500"
              />
            </div>
            <div>
              <label className="block text-xs font-black text-ink-600 uppercase tracking-widest mb-1.5">Subtitle (optional)</label>
              <input
                type="text"
                value={subtitle}
      ...
      {/* Slide List */}
      <div className="bg-white rounded-ds-lg p-6 shadow-ds-soft">
        <h3 className="font-display font-black text-ink-700 mb-4">
          Current Slides <span className="text-ink-400 font-medium">({banners.length})</span>
        </h3>
        ...
        {banners.length === 0 ? (
          ...
          <div className="space-y-3">
            {banners.map((banner, index) => (
              <div key={banner.id} className="flex gap-4 items-center p-4 border border-ink-100 rounded-2xl bg-ink-50/30 hover:bg-ink-50 transition-colors">
                {/* Thumbnails */}
                <div className="flex gap-2 shrink-0">
                  <div className="relative">
                    <img src={banner.desktop_image_url} alt="Desktop" className="w-24 h-14 object-cover rounded-xl border border-ink-200" />
                    <span className="absolute bottom-1 left-1 bg-black/50 text-white text-[9px] px-1 rounded font-bold">Desktop</span>
                  </div>
                  {banner.mobile_image_url && (
                    <div className="relative hidden sm:block">
                      <img src={banner.mobile_image_url} alt="Mobile" className="w-14 h-14 object-cover rounded-xl border border-ink-200" />
                ...
                <div className="flex-1 min-w-0">
                  {banner.title ? (
                    <p className="font-black text-ink-700 text-sm truncate">{banner.title}</p>
                  ) : (
                    <p className="font-bold text-ink-400 text-sm italic">No title</p>
                  )}
                  {banner.subtitle && (
                    <p className="text-xs text-ink-500 truncate">{banner.subtitle}</p>
                  )}
                  <p className="text-[10px] text-ink-400 mt-0.5">Slide {index + 1}</p>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button
                    ...
                    className={`transition-colors ${banner.is_active ? 'text-medical-600' : 'text-ink-300'}`}
                  ...
                  <button
                    ...
                    disabled={index === 0}
                    className="p-1.5 rounded-lg hover:bg-ink-200 disabled:opacity-30 transition-colors text-ink-600"
                    title="Move up"
                  >
                    <ChevronUp size={16} />
                  </button>
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-lg hover:bg-red-50 text-ink-400 hover:text-red-500 disabled:opacity-50 transition-colors"
                    title="Delete slide"
                  >
