...
interface PrescriptionEditorProps {
  initialPatient?: { name: string; age?: number; gender: string } | null;
  onClearInitial?: () => void;
  onSave?: (rx: any) => void;
}
...
export const PrescriptionEditor: React.FC<PrescriptionEditorProps> = ({ initialPatient, onClearInitial, onSave }) => {
  // Mocking the currently logged-in doctor (Dr. Sarah Ahmed)
  ...
  // Data State
  const [selectedChamberId, setSelectedChamberId] = useState<string>(currentDoctor.chambers[0].id);
  ...
  const selectedChamber = currentDoctor.chambers.find(c => c.id === selectedChamberId) || currentDoctor.chambers[0];
  ...
  const handleFinishPrescription = () => {
    ...
    const rxPackage = {
      ...
      medicines: selectedMeds.map(m => ({
        ...
        type: m.medicine.type,
        generic: m.medicine.genericName,
        dose: `${m.morningDose}+${m.noonDose}+${m.nightDose}`,
        instruction: m.instruction,
        duration: `${m.duration} Days`
      })),
      advice,
      doctorName: currentDoctor.name,
      doctorDegrees: currentDoctor.degrees,
      specialty: currentDoctor.specialty,
      bmdc: currentDoctor.bmdcNumber
    };

    if (onSave) {
      onSave(rxPackage);
      // Visual feedback
      alert('Prescription successfully shared with ' + patientName + '\'s portal!');
    }
  };
        ...
        <div className={`flex-1 space-y-4 overflow-y-auto pr-1 md:pr-2 custom-scrollbar pb-40 md:pb-10 ${mobileView === 'preview' ? 'hidden xl:block' : 'block'}`}>
          <GlassCard className="p-4 border-l-4 border-blue-500">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="text-sm font-bold text-slate-700 uppercase">Location</h3>
            </div>
            <select 
              value={selectedChamberId} 
              onChange={(e) => setSelectedChamberId(e.target.value)}
              className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-blue-500 font-medium text-slate-700 cursor-pointer h-12"
            >
              {currentDoctor.chambers.map(chamber => (
          ...
          <GlassCard className="p-4">
            ...
            <div className="space-y-4">
              ...
              <textarea 
                ...
                onChange={e => setDiagnosis(e.target.value.split('\n'))}
              />
            </div>
          </GlassCard>
        ...
        <GlassCard className={`flex-[1.5] flex flex-col h-full bg-white shadow-2xl relative overflow-hidden p-0 rounded-none md:rounded-3xl border-0 md:border md:border-white/60 ${mobileView === 'editor' ? 'hidden xl:flex' : 'flex'} pb-40 xl:pb-0`}>
          ...
          <div className="p-6 md:p-8 border-b-2 flex justify-between items-start" style={{ backgroundColor: `${currentTemplate.themeColor}10`, borderColor: currentTemplate.themeColor }}>
            ...
            <div className="text-right">
              <h2 className="text-sm md:text-xl font-bold text-slate-900">{currentDoctor.name}</h2>
              <p className="text-[10px] md:text-sm text-blue-600 font-bold uppercase tracking-widest">{currentDoctor.specialty}</p>
            </div>
          </div>
